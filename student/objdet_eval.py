...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            label_box = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width,
                                                  label.box.length, label.box.heading)
            ## step 2 : loop over all detected objects
            for row in detections:

                ## step 3 : extract the four corners of the current detection
                _, x, y, z, _, w, l, yaw = row
                detection_box = tools.compute_box_corners(x, y, w, l, yaw)

                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                center_dist_x = label.box.center_x - x
                center_dist_y = label.box.center_y - y
                center_dist_z = label.box.center_z - z

                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                iou = Polygon(label_box).intersection(Polygon(detection_box)).area /\
                      Polygon(label_box).union(Polygon(detection_box)).area
...
def compute_performance_stats(det_performance_all):
    ...
    positives = np.array(pos_negs).sum(axis=0)[0]
    true_positives = np.array(pos_negs).sum(axis=0)[1]
    false_negatives = np.array(pos_negs).sum(axis=0)[2]
    false_positives = np.array(pos_negs).sum(axis=0)[3]
