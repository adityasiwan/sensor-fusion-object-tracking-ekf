...
def show_range_image(frame):
    ...
    range_images, camera_projections, range_image_top_pose = waymo_utils.parse_range_image_and_camera_projection(lidar_data)

    # step 2 : extract the range and the intensity channel from the range image
    range_images = range_images[:, :, :2]

    # step 3 : set values <0 to zero
    range_images[range_images < 0] = 0.0

    # step 4 : map the range channel onto an 8-bit scale and make sure that the full range of values is appropriately considered
    range_channel = range_images[:, :, 0]
    range_channel = range_channel / (range_channel.max() - range_channel.min()) * 255
    range_channel = range_channel.astype(np.uint8)

    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    ri_intensity = range_images[:, :, 1]
    ri_intensity = (ri_intensity / (np.percentile(ri_intensity, 1) - np.percentile(ri_intensity, 99))) * 255
    img_intensity = ri_intensity.astype(np.uint8)

    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    range_img_intensity = np.vstack([range_channel, img_intensity]) #range_img_intensity = cv2.vconcat([range_channel, img_intensity])
    img_range_intensity = range_img_intensity[:, :int(range_img_intensity.shape[1]/2)]
    ####### ID_S1_EX1 END #######
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_cpy[:, 0] = np.int_(np.floor((lidar_pcl[:, 0] - configs.lim_x[0]) / bev_discreet))
    ...
    _, idxs = np.unique(lidar_pcl_cpy[:, 0:2], axis=0, return_index=True)
